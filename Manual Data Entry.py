...
while(True): 
    ...
    team_number = raw_input("Enter team number: ")
    if team_number == "exit":
        break
    match_number = raw_input("Enter match number, type 'c' to cancel: ")
    if match_number == "exit":
        break
    elif match_number != "c":  
        scan = raw_input("Press enter to begin scan, type any other value to cancel: ")
        if scan == "exit":
            break
        elif scan == "":
            keyword = True
